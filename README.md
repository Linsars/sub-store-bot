...
| 按钮 | 功能 |
|------|------|
| 🌐 远程订阅 | 输入订阅 URL，自动拉取并进入格式选择页 |
| 📎 本地订阅 | 发送节点文本/文件，自动解析进入格式选择页 |
| 🌐 UA 轮询 | 配置订阅拉取用的 User-Agent 池 |
| ⏱ 短链时限 | 设置默认时效 + 访问次数上限（统一入口） |
| 📋 我的短链 | 管理已生成的短链（查看详情、复制、删除、修改时效） |
| **格式选择页** | |
| 13 种格式按钮 | 选择输出格式开始转换 |
| ⏱ 本次时限 | 单独设本次转换的有效期和访问次数，不影响主页默认 |
| 🔥 阅后即焚 | 开启后首次访问短链即销毁 |
| 🌀 梅开二度 | 开启后短链以落地页展示（赛博风格 HTML） |
| **短链详情页** | |
| 🔗 打开 | 在浏览器中打开短链 |
| 🔄 刷新 | 实时更新访问次数 |
| ⌛ 修改时效 | 修改有效期限 |
